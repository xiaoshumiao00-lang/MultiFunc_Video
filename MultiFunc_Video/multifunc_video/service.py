...
"""
MultiFunc_Video Core - Service Layer
...
from multifunc_video.services.tts_service import TTSService
from multifunc_video.services.media import MediaService
from multifunc_video.services.image_analysis import ImageAnalysisService
from multifunc_video.services.video_analysis import VideoAnalysisService
from multifunc_video.services.video import VideoService
from multifunc_video.services.frame_processor import FrameProcessor
from multifunc_video.services.persistence import PersistenceService
from multifunc_video.services.history_manager import HistoryManager
from multifunc_video.pipelines.standard import StandardPipeline
from multifunc_video.pipelines.custom import CustomPipeline
from multifunc_video.pipelines.asset_based import AssetBasedPipeline


class MultiFuncVideoCore:
    """
    MultiFunc_Video Core - Service Layer
    ...
    Usage:
        ...
        media = await multifunc_video.media(prompt="a cat")
        
        # Check active capabilities
        print(f"Using LLM: {multifunc_video.llm.active}")
        print(f"Available TTS: {multifunc_video.tts.available}")
    
    Architecture (Simplified):
        MultiFuncVideoCore (this class)
    ...
    def __init__(self, config_path: str = "config.yaml"):
        """
        Initialize MultiFunc_Video Core
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("MultiFunc_Video already initialized")
            return
        
        logger.info("🚀 Initializing MultiFunc_Video...")
        ...
        logger.info("✅ MultiFunc_Video initialized successfully\n")
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "MultiFunc_Video")
    
    def __repr__(self) -> str:
        """String representation"""
        status = "initialized" if self._initialized else "not initialized"
        pipelines = f"pipelines={list(self.pipelines.keys())}" if self._initialized else ""
        return f"<MultiFuncVideoCore project={self.project_name!r} status={status} {pipelines}>"
