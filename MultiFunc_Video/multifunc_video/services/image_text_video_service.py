...
class ImageTextVideoService:
    ...
    def _run_video_generator(self, wrapper_path: Path, params: Dict,
        ...
        # 阶段定义（与 smart_video.generate_sync 的输出关键字对应）
        phases = [
                ...
                if progress_callback:
                    for step, (message, keywords) in enumerate(phases):
                        if any(kw in line for kw in keywords):
                            ...
                            progress_callback(current_step, len(phases), message)
                            break
    ...
    def generate(self,
        ...
        if ref_audio_bytes and ref_audio_filename:
            ...
            ref_audio_in_project = self._copy_ref_audio_to_voices(saved_ref_path)
        ...
        try:
            # 运行视频生成
            if progress_callback:
                progress_callback(0, 9, "启动 video-generator")
