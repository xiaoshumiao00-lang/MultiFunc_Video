...
from web.state.cache import load_state_into_session
...
def init_session_state():
    ...
    # Load persisted UI cache into session state so inputs survive restarts
    load_state_into_session()
...
def get_multifunc_video():
    """
    Get initialized MultiFunc_Video instance with proper caching and cleanup
    ...
    need_recreate = False
    if 'multifunc_video' not in st.session_state:
        need_recreate = True
        logger.info("Creating new MultiFuncVideoCore instance (first time)")
    elif st.session_state.get('multifunc_video_config_hash') != config_hash:
        need_recreate = True
        logger.info("Configuration changed, recreating MultiFuncVideoCore instance")
        # Cleanup old instance
        old_core = st.session_state.multifunc_video
        try:
            run_async(old_core.cleanup())
        except Exception as e:
            logger.warning(f"Failed to cleanup old MultiFuncVideoCore: {e}")
    
    if need_recreate:
        # Create and initialize new instance
        multifunc_video = MultiFuncVideoCore()
        run_async(multifunc_video.initialize())
        
        # Cache in session state
        st.session_state.multifunc_video = multifunc_video
        st.session_state.multifunc_video_config_hash = config_hash
        logger.info("✅ MultiFuncVideoCore initialized and cached")
    else:
        multifunc_video = st.session_state.multifunc_video
        logger.debug("Reusing cached MultiFuncVideoCore instance")
    
    return multifunc_video
