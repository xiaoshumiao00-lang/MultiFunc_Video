...
from web.state.cache import clear_all as clear_ui_cache
    ...
    with st.expander(tr("settings.title"), expanded=not is_configured):
        ...
        with llm_col:
            with st.container(border=True):
                ...
                from multifunc_video.llm_presets import get_preset_names, get_preset, find_preset_by_base_url_and_model
                ...
                if load_clicked:
                    if llm_api_key and llm_base_url:
                        try:
                            from multifunc_video.utils.llm_util import fetch_available_models
                ...
                if test_clicked:
                    if llm_api_key and llm_base_url:
                        try:
                            from multifunc_video.utils.llm_util import test_llm_connection
                            with st.spinner(tr("settings.llm.loading_models")):
        ...
        with col2:
            if st.button(tr("btn.reset_config"), use_container_width=True, key="reset_config_btn"):
                # Reset to default
                from multifunc_video.config.schema import MultiFuncVideoConfig
                config_manager.config = MultiFuncVideoConfig()
                config_manager.save()
                st.success(tr("status.config_reset"))
                safe_rerun()

        with col3:
            if st.button(tr("btn.clear_cache"), use_container_width=True, key="clear_cache_btn"):
                clear_ui_cache()
                st.success(tr("status.cache_cleared"))
                safe_rerun()
