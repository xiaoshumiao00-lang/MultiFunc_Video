...
def render_style_config(multifunc_video):
    ...
    with st.container(border=True):
        ...
        # ================================================================
            ...
            tts_workflows = multifunc_video.tts.list_workflows()
            ...
            tts_workflow_display = cached_selectbox(
                "TTS Workflow",
            ...
            if ref_audio_file is not None:
                ...
                cached_paths = cache_uploaded_files("ref_audio_upload", ref_audio_file if isinstance(ref_audio_file, list) else [ref_audio_file])
                if cached_paths:
                    ref_audio_path = Path(cached_paths[0])
            else:
                cached_paths = get_cached_file_paths("ref_audio_upload")
                if cached_paths:
                    ref_audio_path = Path(cached_paths[0])
                    st.info(f"🔊 {tr('cache.using_cached_ref_audio')}: {ref_audio_path.name}")
                    st.audio(str(ref_audio_path))
                    if st.button(tr("cache.clear_ref_audio"), key="clear_ref_audio_upload"):
                        clear_cached_files("ref_audio_upload")
                        st.rerun()
    ...
    with st.container(border=True):
        ...
        from multifunc_video.utils.template_util import get_templates_grouped_by_size_and_type, get_template_type
        ...
        selected_template_type = cached_radio(
        ...
        template_config = multifunc_video.config.get("template", {})
        ...
        # Initialize selected template from cache/session state if not exists
        from web.state.cache import get as cache_get, set as cache_set
        if 'selected_template' not in st.session_state:
            cached_template = cache_get("selected_template", type_specific_default)
            st.session_state['selected_template'] = cached_template
        ...
        if last_template_type != selected_template_type:
            ...
            cache_set("selected_template", type_specific_default)
                        ...
                        for idx, template in enumerate(all_templates):
                            ...
                            with cols[col_idx]:
                                ...
                                if st.button(
                                    ...
                                    cache_set("selected_template", template.template_path)
                                    st.rerun()
            else:
        ...
        from multifunc_video.utils.template_util import parse_template_size
        ...
        from multifunc_video.services.frame_html import HTMLFrameGenerator
        # Resolve template path to support both data/templates/ and templates/
        from multifunc_video.utils.template_util import resolve_template_path
        ...
        from multifunc_video.utils.template_util import get_template_type
        ...
        if custom_params_for_video:
            ...
            def _render_custom_param(param_name, config):
                ...
                if param_type == 'text':
                    return cached_text_input(label, key=widget_key, default=default)
                elif param_type == 'number':
                    return cached_number_input(label, key=widget_key, value=default)
                elif param_type == 'color':
                    return cached_color_picker(label, key=widget_key, default=default)
                elif param_type == 'bool':
                    return cached_checkbox(label, key=widget_key, value=default)
                return default

            # Left column parameters
            with video_custom_col1:
                for param_name, config in param_items[:mid_point]:
                    custom_values_for_video[param_name] = _render_custom_param(param_name, config)
        ...
        with st.expander(tr("template.preview_title"), expanded=False):
            ...
            with col1:
                preview_title = cached_text_input(
                    tr("template.preview_param_title"),
                    key="preview_title",
                    default=tr("template.preview_default_title")
                )
                preview_image = cached_text_input(
                    tr("template.preview_param_image"),
                    key="preview_image",
                    default="resources/example.png",
                    help=tr("template.preview_image_help")
                )

            with col2:
                preview_text = cached_text_area(
                    tr("template.preview_param_text"),
                    key="preview_text",
                    default=tr("template.preview_default_text"),
                    height=100
                )
            
            # Info: Size is auto-determined from template
            from multifunc_video.utils.template_util import parse_template_size, resolve_template_path
    ...
    if template_requires_media:
        ...
        with st.container(border=True):
            ...
            all_workflows = multifunc_video.media.list_workflows()
            ...
            workflow_display = cached_selectbox(
                ...
                options=workflow_options if workflow_options else ["No workflows found"],
            ...
            prompt_prefix = cached_text_area(
                tr('style.prompt_prefix'),
                key="media_prompt_prefix",
            ...
            with st.expander(preview_title, expanded=False):
                ...
                test_prompt = cached_text_input(
                    test_prompt_label,
                    key="style_test_prompt",
                    default=test_prompt_value,
                ...
                if st.button(preview_button_label, key="preview_style", use_container_width=True):
                    ...
                    with st.spinner(previewing_text):
                        try:
                            from multifunc_video.utils.prompt_helper import build_image_prompt
                            ...
                            media_result = run_async(multifunc_video.media(
                                prompt=final_prompt,
