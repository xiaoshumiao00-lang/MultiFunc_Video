...
class DigitalHumanPipelineUI(PipelineUI):
    ...
    def render(self, multifunc_video: Any):
        ...
        with left_col:
            ...
            style_params = render_style_config(multifunc_video, key_prefix="digital_human")
    ...
    def render_digital_human_input(self) -> dict:
        ...
        with st.container(border=True):
            ...
            character_cache_key = "character_files"
            if uploaded_files:
                ...
                if cached_paths:
                    ...
                    if st.button(tr("cache.clear_files"), key="clear_character_files"):
    ...
    def render_digital_human_mode(self, character_asset_paths: list) -> dict:
        with st.container(border=True):
            ...
            if mode == "digital":
                ...
                # Persist uploaded files and restore cached files across restarts
                goods_asset_paths = []
                goods_cache_key = "digital_files"
                if uploaded_files:
                    goods_asset_paths = cache_uploaded_files(goods_cache_key, uploaded_files)
                    ...
                    cached_paths = get_cached_file_paths(goods_cache_key)
                    if cached_paths:
                        goods_asset_paths = cached_paths
                        st.info(f"📁 {tr('cache.using_cached_files', count=len(cached_paths))}")
                        with st.expander(tr("digital_human.assets.preview"), expanded=True):
                            cols = st.columns(3)
                            for i, path in enumerate(goods_asset_paths):
                                with cols[i % 3]:
                                    ext = Path(path).suffix.lower()
                                    if ext in [".jpg", ".jpeg", ".png", ".webp"]:
                                        st.image(path, caption=Path(path).name, use_container_width=True)
                        if st.button(tr("cache.clear_files"), key="clear_digital_files"):
                            clear_cached_files(goods_cache_key)
                            st.rerun()
                    else:
                        st.info(tr("digital_human.assets.goods_empty_hint"))

                goods_text = cached_text_area(
