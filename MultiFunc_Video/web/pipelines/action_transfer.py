...
class ActionTransferPipelineUI(PipelineUI):
    ...
    def render(self, multifunc_video: Any):
        ...
        # ====================================================================
        with left_col:
            video_params = self.render_action_transfer_video_input(multifunc_video)
            render_version_info()
        ...
        # ====================================================================
        with middle_col:
            assets_params = self.render_action_transfer_assets_input(multifunc_video)
    ...
    def render_action_transfer_video_input(self, multifunc_video) -> dict:
        with st.container(border=True):
            ...
            video_asset_paths = []
            video_cache_key = "action_reference_files"
            if uploaded_files:
                video_asset_paths = cache_uploaded_files(video_cache_key, uploaded_files)
                ...
                cached_paths = get_cached_file_paths(video_cache_key)
                if cached_paths:
                    video_asset_paths = cached_paths
                    st.info(f"📁 {tr('cache.using_cached_files', count=len(cached_paths))}")
                    with st.expander(tr("action_transfer.assets.preview"), expanded=True):
                        cols = st.columns(3)
                        for i, path in enumerate(video_asset_paths):
                            with cols[i % 3]:
                                ext = Path(path).suffix.lower()
                                if ext in [".mp4", ".mkv", ".mov"]:
                                    st.video(path)
                    if st.button(tr("cache.clear_files"), key="clear_action_reference_files"):
                        clear_cached_files(video_cache_key)
                        st.rerun()
                else:
    ...
    def render_action_transfer_assets_input(self, multifunc_video) -> dict:
        with st.container(border=True):
            ...
            # Persist uploaded files and restore cached files across restarts
            image_asset_paths = []
            image_cache_key = "image_files"
            if uploaded_files:
                image_asset_paths = cache_uploaded_files(image_cache_key, uploaded_files)
                ...
                cached_paths = get_cached_file_paths(image_cache_key)
                if cached_paths:
                    image_asset_paths = cached_paths
                    st.info(f"📁 {tr('cache.using_cached_files', count=len(cached_paths))}")
                    with st.expander(tr("action_transfer.assets.preview"), expanded=True):
                        cols = st.columns(3)
                        for i, path in enumerate(image_asset_paths):
                            with cols[i % 3]:
                                ext = Path(path).suffix.lower()
                                if ext in [".jpg", ".jpeg", ".png", ".webp"]:
                                    st.image(path, caption=Path(path).name, use_container_width=True)
                    if st.button(tr("cache.clear_files"), key="clear_image_files"):
                        clear_cached_files(image_cache_key)
                        st.rerun()
                else:
